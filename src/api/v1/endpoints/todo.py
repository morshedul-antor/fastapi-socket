...
async def create_todo(data_in: TodoBase, db: Session = Depends(get_db), current_user: Session = Depends(logged_in)):
    todo = await todo_service.create_todo(db=db, data_in=data_in, user_id=current_user.id)
